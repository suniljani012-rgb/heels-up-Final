...
export const InstagramFeedWidget: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const existingScript = document.getElementById('trustindex-feed-script');
    if (existingScript) {
      existingScript.remove();
    }

    const script = document.createElement('script');
    script.id = 'trustindex-feed-script';
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 md:px-8 mt-24 mb-24 select-none">
      ...
      <div ref={containerRef} className="w-full flex justify-center min-h-[300px] overflow-hidden rounded-2xl">
        {/* Trustindex Instagram feed automatically embeds here */}
      </div>
    </section>
  );
};
